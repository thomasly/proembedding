...
def split_dataset_with_kfold(x, y, k_th, k_fold, augmentation=False):
    d_len = len(x)
    chunk_len = int(d_len/k_fold)
    chunk_start = chunk_len * k_th
    chunk_end = chunk_len * (k_th + 1)
...
def myargs():
    ...
    parser.add_argument('--log', help=
                        'path to the directory saving training results')
    args = parser.parse_args()
    return args

if __name__ == "__main__":
    args = myargs()
    train_deepdrug(args.bs, args.lr, args.epoch, args.input, args.log,
                   k_fold=args.fold)
